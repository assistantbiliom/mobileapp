...
function AppGate() {
  ...
  const storeData = async (localToken: string) => {
    try {
      await AsyncStorage.setItem('mSessionToken', localToken);
      console.log("mSessionToken stored locally");
    } catch {
      console.log("Error storing mSessionToken");
    }
  };
  ...
  if (mSessionToken && mSessionToken !== '0') {
    return (
        ...
        <View style={globalStyles.loginPage.socialcontainer}>
          {socialPlatforms.map((platform, index) => (
            <TouchableOpacity
              ...
              onPress={() => console.log("TODO: handle login for", platform.name)}
            >
